...
namespace RPC
{
	...
	struct PlayerConnect : NetworkPacketBase<25, NetworkPacketType::RPC, OrderingChannel_SyncRPC>
	{
		...
		bool read(NetworkBitStream& bs)
		{
			...
			// Legacy detection first (exact-fit payload):
			// ChallengeResponse, VersionFlag, VersionNumber, Modded, Name.
			bs.resetReadPointer();
			uint32_t legacyChallenge = 0;
			uint8_t versionFlag = 0;
			uint32_t legacyVersion = 0;
			uint8_t legacyModded = 0;
			HybridString<MAX_PLAYER_NAME + 1> legacyName;
			if (bs.readUINT32(legacyChallenge)
				&& bs.readUINT8(versionFlag)
				&& bs.readUINT32(legacyVersion)
				&& bs.readUINT8(legacyModded)
				&& bs.readDynStr8(legacyName)
				&& bs.GetNumberOfUnreadBits() == 0)
			{
				ChallengeResponse = legacyChallenge;
				VersionNumber = legacyVersion;
				Modded = legacyModded;
				Name = legacyName;
				(void)versionFlag;
				Key = "0";
			...
			bs.resetReadPointer();
			// Preferred format (open.mp / modern).
			if (bs.readUINT32(VersionNumber)
				&& bs.readUINT8(Modded)
				&& bs.readDynStr8(Name)
				&& bs.readUINT32(ChallengeResponse)
				&& bs.readDynStr8(Key)
				&& bs.readDynStr8(VersionString))
			{
				uint32_t dummyVar = 0;
