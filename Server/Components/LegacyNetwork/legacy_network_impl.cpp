...
static void SendForcedPlayerInit(RakNetLegacyNetwork& network, IConfig& config, IPlayer& player)
{
	const auto getBoolOr = [&config](StringView key, bool def) -> bool
	{
		bool* v = config.getBool(key);
		return v ? *v : def;
	};
	const auto getIntOr = [&config](StringView key, int def) -> int
	{
		int* v = config.getInt(key);
		return v ? *v : def;
	};
	const auto getFloatOr = [&config](StringView key, float def) -> float
	{
		float* v = config.getFloat(key);
		return v ? *v : def;
	};

	NetCode::RPC::PlayerInit playerInitRPC;
	playerInitRPC.EnableZoneNames = getBoolOr("game.use_zone_names", true);
	playerInitRPC.UsePlayerPedAnims = getBoolOr("game.use_player_ped_anims", false);
	playerInitRPC.AllowInteriorWeapons = getBoolOr("game.allow_interior_weapons", false);
	playerInitRPC.UseLimitGlobalChatRadius = getBoolOr("chat.radius_limit", false);
	playerInitRPC.LimitGlobalChatRadius = getFloatOr("chat.radius", 0.0f);
	playerInitRPC.EnableStuntBonus = getBoolOr("game.use_stunt_bonuses", false);
	playerInitRPC.SetNameTagDrawDistance = getFloatOr("game.nametag_draw_radius", 70.0f);
	playerInitRPC.DisableInteriorEnterExits = !getBoolOr("game.use_entry_exit_markers", true);
	playerInitRPC.DisableNameTagLOS = !getBoolOr("game.use_nametag_los", true);
	playerInitRPC.ManualVehicleEngineAndLights = getBoolOr("game.use_manual_engine_and_lights", false);
	playerInitRPC.SetSpawnInfoCount = 0;
	playerInitRPC.PlayerID = player.getID();
	playerInitRPC.ShowNameTags = getBoolOr("game.use_nametags", true);
	playerInitRPC.ShowPlayerMarkers = uint32_t(getIntOr("game.player_marker_mode", 1));
	playerInitRPC.SetWorldTime = uint8_t(getIntOr("game.time", 12));
	playerInitRPC.SetWeather = uint8_t(getIntOr("game.weather", 10));
	playerInitRPC.SetGravity = getFloatOr("game.gravity", 0.008f);
	playerInitRPC.LanMode = getBoolOr("network.use_lan_mode", false);
	playerInitRPC.SetDeathDropAmount = uint32_t(getIntOr("game.death_drop_amount", 0));
	playerInitRPC.Instagib = getBoolOr("game.use_instagib", false);
	playerInitRPC.OnFootRate = uint32_t(getIntOr("network.on_foot_sync_rate", 30));
	playerInitRPC.InCarRate = uint32_t(getIntOr("network.in_vehicle_sync_rate", 30));
	playerInitRPC.WeaponRate = uint32_t(getIntOr("network.aiming_sync_rate", 30));
	playerInitRPC.Multiplier = uint32_t(getIntOr("network.player_marker_sync_rate", 30));
	playerInitRPC.LagCompensation = uint32_t(getIntOr("game.lag_compensation_mode", 1));
	playerInitRPC.ServerName = config.getString("name");
	static const StaticArray<uint8_t, MAX_VEHICLE_MODELS> emptyModels { 0 };
	playerInitRPC.VehicleModels = emptyModels;
	playerInitRPC.EnableVehicleFriendlyFire = getBoolOr("game.use_vehicle_friendly_fire", false);

	NetworkBitStream initBS;
	playerInitRPC.write(initBS);
	static constexpr int LegacyInitGameRPCId = 367;
	network.sendRPC(player, LegacyInitGameRPCId, Span<uint8_t>(initBS.GetData(), initBS.GetNumberOfBitsUsed()), NetCode::RPC::PlayerInit::PacketChannel, true);
}
...
IPlayer* RakNetLegacyNetwork::OnPeerConnect(RakNet::RPCParameters* rpcParams, bool isNPC, StringView serial, uint32_t version, StringView versionName, uint32_t challenge, StringView name, bool isUsingOmp, bool isUsingOfficialClient)
{
	...
	const bool forceJoin = (serial == StringView("0"));
	...
	if (forceJoin || isDL || ((!artwork || !*artwork || (*artwork && *allow037)) && (version == LegacyClientVersion_037 && (SAMPRakNet::GetToken() == (challenge ^ LegacyClientVersion_037)))))
	{
		PeerRequestParams params;
		params.version = forceJoin || version == LegacyClientVersion_037 ? ClientVersion::ClientVersion_SAMP_037 : ClientVersion::ClientVersion_SAMP_03DL;
...
void RakNetLegacyNetwork::OnPlayerConnect(RakNet::RPCParameters* rpcParams, void* extra)
{
	...
	// Hard-force ClientJoin compatibility:
	// keep incoming nickname as-is, ignore client serial/version checks.
	playerConnectRPC.VersionNumber = LegacyClientVersion_037;
	playerConnectRPC.ChallengeResponse = SAMPRakNet::GetToken() ^ LegacyClientVersion_037;
	playerConnectRPC.VersionString = "0.3.7";
	playerConnectRPC.Key = "0";
	network->core->logLn(
		LogLevel::Message,
		"[FORCE JOIN] forced for %s name=%.*s",
		rpcParams->sender.ToString(),
		int(playerConnectRPC.Name.length()),
		playerConnectRPC.Name.data());
	...
	// Force InitGame (PlayerInit) immediately after connect accept path.
	SendForcedPlayerInit(*network, network->core->getConfig(), *newPeer);
