    ...
    Decrypt(uint8_t const* src, int len, uint16_t port)
{
    static const uint8_t
        rawSBox[256]
        = {
              
          };
    static uint8_t invSBox[256];
    static bool invInited = false;

    if (!invInited)
    {
        for (int i = 0; i < 256; ++i)
        {
            invSBox[rawSBox[i]] = static_cast<uint8_t>(i);
        }
        invInited = true;
    }

    const uint16_t key = static_cast<uint16_t>(port ^ 0x5555);
    for (int i = 0; i < len; ++i)
    {
        const uint8_t xorKey = (i % 2 == 0) ? static_cast<uint8_t>(key >> 8) : static_cast<uint8_t>(key & 0xFF);
        decryptBuffer_[i] = invSBox[static_cast<uint8_t>(src[i] ^ xorKey)];
    }
